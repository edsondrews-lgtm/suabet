...
export async function extrairBilhete(imagemBase64, mimeType, captionTexto = "") {
  ...
  if (!apiKey) {
    console.error("[EXTRACAO] VITE_GEMINI_API_KEY não encontrada");
    throw new Error("VITE_GEMINI_API_KEY não configurada");
  }
  console.log(`[EXTRACAO] Chamando Gemini... mime=${mimeType}, caption="${captionTexto.slice(0,50)}"`);
  ...
  if (!res.ok) {
    console.error("[EXTRACAO] Gemini erro:", res.status, JSON.stringify(data?.error || data).slice(0, 300));
    throw new Error(data?.error?.message || `Gemini erro ${res.status}`);
  }

  const text = data.candidates?.[0]?.content?.parts?.[0]?.text || "";
  console.log("[EXTRACAO] Resposta OK, parseando...");
